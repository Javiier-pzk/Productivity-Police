...
export default function CP1Details({ route }) {
  const checkpoint1 = route.params;
  ...
  const [rating, setRating] = useState(
    cp1.Rating !== undefined
      ? cp1.Rating
      : checkpoint1.Rating !== undefined
      ? checkpoint1.Rating
      : 5
  );

  const reflectionText =
    "Write down your thoughts on how you did for Checkpoint 1! \n";
  ...
  const pushNotes = (reflectionNote, improvementNote, rating) => {
    ...
    Goals.editCheckpoint1(
      newCheckpoint1,
      { userId: Authentication.getCurrentUserId(), goalId: checkpoint1.id },
      () => {},
      (error) => Alert.alert(error.message)
    );
    Alert.alert("Success!", "Notes for Checkpoint 1 have been updated");
  };
  ...
  return (
    <View style={globalStyles.container}>
        ...
        <ScrollView style={{ flex: 1 }}>
          <Card>
            <View>
              ...
              <Text style={styles.bodyText}>{checkpoint1.Daily}</Text>
            </View>
            <View style={styles.body}>
              <Text style={styles.title}> Weekly </Text>
              <Text style={styles.bodyText}>{checkpoint1.Weekly}</Text>
            </View>
            {checkpoint1.Monthly !== "" ? (
              <View style={styles.body}>
                <Text style={styles.title}> Monthly </Text>
                <Text style={styles.bodyText}>{checkpoint1.Monthly}</Text>
              </View>
            ) : null}
            {checkpoint1.Misc !== "" ? (
              <View style={styles.body}>
                <Text style={styles.title}> Miscellaneous </Text>
                <Text style={styles.bodyText}>{checkpoint1.Misc}</Text>
              </View>
            ) : null}
            <View style={styles.dateTime}>
              <Text style={globalStyles.titleText}>
                {checkpoint1.DateTimeFormatted}
              </Text>
            </View>
          </Card>
        </ScrollView>
      ...
      {checkpoint1.completed && showNotes && (
        <Formik
          initialValues={{
            ReflectionNote:
              cp1.ReflectionNote !== undefined
                ? cp1.ReflectionNote
                : checkpoint1.ReflectionNote !== undefined
                ? checkpoint1.ReflectionNote
                : "",
            ImprovementNote:
              cp1.ImprovementNote !== undefined
                ? cp1.ImprovementNote
                : checkpoint1.ImprovementNote !== undefined
                ? checkpoint1.ImprovementNote
                : "",
          }}
          onSubmit={(values) => {
        ...
        >
          {(props) => (
            <KeyboardAwareScrollView
              ...
              extraHeight={300}
            >
              <Text style={styles.notesText}> Checkpoint 1 Notes: </Text>
      ...
      {showNotes && !checkpoint1.completed && (
