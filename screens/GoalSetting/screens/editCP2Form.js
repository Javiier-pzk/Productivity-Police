...
export default function CreateCP2Form({ navigation, route }) {
  ...
  const [dateTime, setDateTime] = useState(
    ...
    ? moment(cp2.DateTimeFormatted, 'Do MMMM YYYY, h:mm a').toDate()
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={globalStyles.container}>
        ...
        <Formik
          initialValues={{ 
            Daily: cp2 === null ? '' : cp2.Daily !== undefined ? cp2.Daily : Daily === undefined ? '' : Daily,
            Weekly: cp2 === null ? '': cp2.Weekly !== undefined ? cp2.Weekly : Weekly === undefined ? '' : Weekly,
            Monthly: cp2 === null ? '' : cp2.Monthly !== undefined ? cp2.Monthly : Monthly === undefined ? '' : Monthly,
            Misc: cp2 === null ? '' : cp2.Misc !== undefined ? cp2.Misc : Misc === undefined ? '' : Misc,
          }}
          validationSchema={validationSchema}
          onSubmit={(values) => {
            if (moment(GoalDateTimeFormatted, 'Do MMMM YYYY, h:mm a').diff(moment(dateTime), 'minutes', true) <= 0) {
              Alert.alert('Invalid date or time', 'You cannot schedule Checkpoint 2 after your goal completion date and time')
            } else if (moment(dateTime).diff(moment(currentTime), 'minutes', true) <= 0) {
              Alert.alert('Invalid date or time', 
                'Checkpoint 2 completion date and time must be some time in the future');
            } else {
              values.DateTimeFormatted = moment(dateTime).format('Do MMMM YYYY, h:mm a');
              values.completed = false;
              dispatch(setCP2(values));
              navigation.goBack();
            }
          }}
        >
          {(props) => (
            <SafeAreaView style={styles.modalContent}>
              <View style={styles.header}>
                ...
                <Text style={styles.headerText}> Edit Checkpoint 2 </Text>
              ...
              <ScrollView style={styles.scrollView}>
                ...
                <View style={{ marginTop: 5, marginBottom: 30 }}>
                  ...
                  <List.Item
                    ...
                    description="Time to complete checkpoint 2"
                    descriptionStyle={styles.description}
                    onPress={() => {
                      showTimepicker();
                      setShowDate(false);
