...
export default function CP2Details({ route }) {
  const checkpoint2 = route.params;
  const { cp2 } = useSelector((state) => state.checkpoint2);
  const dispatch = useDispatch();
  const [showDetails, setShowDetails] = useState(true);
  const [showNotes, setShowNotes] = useState(false);
  const [rating, setRating] = useState(
    cp2.Rating !== undefined
      ? cp2.Rating
      : checkpoint2.Rating !== undefined
      ? checkpoint2.Rating
      : 5
  );

  const reflectionText =
    "Write down your thoughts on how you did for Checkpoint 2! \n";
  ...
  const pushNotes = (reflectionNote, improvementNote, rating) => {
    ...
    Goals.editCheckpoint2(
      newCheckpoint2,
      { userId: Authentication.getCurrentUserId(), goalId: checkpoint2.id },
      () => {},
      (error) => Alert.alert(error.message)
    );
    Alert.alert("Success!", "Notes for Checkpoint 2 have been updated");
  };
  ...
  return (
    <View style={globalStyles.container}>
        ...
        <ScrollView style={{ flex: 1 }}>
          <Card>
            <View>
              ...
              <Text style={styles.bodyText}>{checkpoint2.Daily}</Text>
            </View>
            <View style={styles.body}>
              <Text style={styles.title}> Weekly </Text>
              <Text style={styles.bodyText}>{checkpoint2.Weekly}</Text>
            </View>
            {checkpoint2.Monthly !== "" ? (
              <View style={styles.body}>
                <Text style={styles.title}> Monthly </Text>
                <Text style={styles.bodyText}>{checkpoint2.Monthly}</Text>
              </View>
            ) : null}
            {checkpoint2.Misc !== "" ? (
              <View style={styles.body}>
                <Text style={styles.title}> Miscellaneous </Text>
                <Text style={styles.bodyText}>{checkpoint2.Misc}</Text>
              </View>
            ) : null}
            <View style={styles.dateTime}>
              <Text style={globalStyles.titleText}>
                {checkpoint2.DateTimeFormatted}
              </Text>
            </View>
          </Card>
        </ScrollView>
      ...
      {checkpoint2.completed && showNotes && (
        <Formik
          initialValues={{
            ReflectionNote:
              cp2.ReflectionNote !== undefined
                ? cp2.ReflectionNote
                : checkpoint2.ReflectionNote !== undefined
                ? checkpoint2.ReflectionNote
                : "",
            ImprovementNote:
              cp2.ImprovementNote !== undefined
                ? cp2.ImprovementNote
                : checkpoint2.ImprovementNote !== undefined
                ? checkpoint2.ImprovementNote
                : "",
          }}
          onSubmit={(values) => {
        ...
        >
          {(props) => (
            <KeyboardAwareScrollView
              ...
              extraHeight={300}
            >
              <Text style={styles.notesText}> Checkpoint 2 Notes: </Text>
      ...
      {showNotes && !checkpoint2.completed && (
